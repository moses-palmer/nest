...
" Allows repeated invocation of a command to force execute it.
"
" If this function is executed twice within one second, it will return v:true,
" otherwise v:false.
function! lib#allow_force(bufnr, state_var)
    let l:previous = getbufvar(a:bufnr, a:state_var)
    if empty(l:previous) || reltimefloat(reltime(l:previous)) > 1.0
        call setbufvar(a:bufnr, a:state_var, reltime())
        return v:false
    else
        return v:true
    endif
...
function! lib#kill_current_buffer(dir)
    ...
    if getbufvar(l:bufnr, '&mod') && !lib#allow_force(l:bufnr, '__kill_ts')
...
function! lib#kill_other_buffers(dir)
    " While we have siblings in the direction specified, kill all unmodified
    " buffers
