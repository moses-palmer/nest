...
@main.remover
def remove(me: Twig):
    for item in dconf.list(me.name, '{}/'.format(PATH)):
        path = '{}/{}'.format(PATH, item)
        name = dconf.read(me.name, '{}name'.format(path))
        if any(name == binding['name'] for binding in BINDINGS):
            dconf.reset(me.name, path)
...
def _list_missing(me: Twig) -> [dict]:
    ...
    """
    current_names = [
        dconf.read(me.name, '{}/{}name'.format(PATH, binding))
        for binding in dconf.list(me.name, '{}/'.format(PATH))]
    return [
        binding
