...
#: The version reposted from crates installed from a repository.
NULL_VERSION = '0.0.0'
...
def crate(
    ...
    def main(me: Twig):
        ...
        if from_repository is None:
            ...
            _run(
                me,
                BIN_CARGO, 'install', '--git=${repository}', '--tag=${tag}',
                *feature_args,
                me.name,
                interactive=False,
...
def _run(me: Twig, binary: str, *args: str, **kwargs: str):
    ...
    """
    return me.run(_qualify(binary), *args, **kwargs)
...
def _installed_crates() -> Dict[str, str]:
    ...
    try:
        return {
            crate: version if version != NULL_VERSION else tag
