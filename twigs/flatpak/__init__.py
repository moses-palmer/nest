...
def _is_installed(me: Twig, flatpak: Twig) -> bool:
    ...
    return flatpak.run(
        ...
        check=True,
        silent=True,
        flatpak=me.configuration.flatpak.packages[flatpak.name].ref()) \
        and binary.exists() \
...
def completer(me: Twig) -> List[str]:
    me.run('flatpak', 'update', '--user', '--noninteractive')
...
def _ref(me: Twig, flatpak: Twig) -> str:
    ...
    :return: a ref
    """
    return me.configuration.flatpak.packages[flatpak.name].ref()
    ...
    return dict(
        (
            ...
            Path(Twig.interpolate(
                me.configuration.flatpak.packages[flatpak.name].links[k](),
                os.getenv,
            )).resolve(),
        )
        for k in me.configuration.flatpak.packages[flatpak.name].links)
...
def _env(me: Twig, flatpak: Twig) -> Dict[Path, Path]:
    ...
    """
    environment = me.configuration.flatpak.packages[flatpak.name].env() or {}
    return dict(
        (k, me.configuration.flatpak.packages[flatpak.name].env[k]())
