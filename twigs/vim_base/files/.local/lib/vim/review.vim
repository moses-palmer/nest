...
function! s:open_modifications()
    ...
    " Find the revisions
    if !search(g:INDEX_RE[0], 'cW')
        return
    endif
    let l:m = s:match(line('.'), g:INDEX_RE)
    if empty(l:m)
    ...
    if !search(g:FILE_A_RE[0], 'cW')
        return
    endif
    let l:m = s:match(line('.'), g:FILE_A_RE)
    if empty(l:m)
    ...
    if !search(g:FILE_B_RE[0], 'cW')
        return
    endif
    let l:m = s:match(line('.'), g:FILE_B_RE)
    if empty(l:m)
        echo('Failed to locate files.')
        return
    else
