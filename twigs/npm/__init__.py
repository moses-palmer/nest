...
def package(
    ...
    :return: the twig
    """
    def specification(me: Twig) -> str:
        try:
            return '{}@{}'.format(me.name, me.stored_version)
    ...
    def versions(me: Twig) -> List[Optional[str]]:
        try:
            return [json.loads(me.run(
                BIN_NPM, 'outdate', '--global', '--json', me.name,
                capture=True))[me.name]['latest']]
    ...
    def main(me: Twig):
        me.run(
            ...
            '--ignore-scripts',
            '${specification}',
            specification=specification(me),
            check=True,
            silent=True)

    @main.checker
    def is_installed(me: Twig) -> bool:
        version = _installed_packages(me).get(me.name, None)
        try:
