...
def is_versioned(me: Twig) -> bool:
    """Whether a twig is versioned.

    A twig is versioned if all files in the strack trace to where it was
    instantiated that are located within this repository are versioned.

    :param twig: The twig to check.

    :return: whether this twig is versioned
    """
    return all(
        command(
            me,
            'ls-files', '--error-unmatch', str(p),
            check=True,
            silent=True)
        for p in me.implementation)
