...
class Twig:
    ...
    def _files(self, rel: Path) -> List[Path]:
        ...
        :return: a list of files and possibly directories.
        """
        if self.user_source is not None and self._copy_files:
            return self.list_files(self.user_source / rel)
        else:
            return []
    ...
    def run(
            ...
            cwd=None, interactive=True, silent=False, **kwargs) -> Union[
                bool,
                str,
                subprocess.Popen,
            ]:
    ...
    def template(
        ...
        **kwargs,
        ...
        """
        try:
            data = source.read_text().format(**kwargs)
        except Exception as e:
            raise NestException(
                'failed to generate templated file from {} for twig {}: {}',
                str(source), self.name, e)
        self.file(io.StringIO(data), target, mode or source.stat().st_mode)
