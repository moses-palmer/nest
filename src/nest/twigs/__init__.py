...
from .. import ROOT, NestException, ui
...
class Twig:
    ...
    def run(
            self, *args, check=False, capture=False, stream=False,
    ...
    def run_progress(self, *args, progress_re: re.Pattern=None, **kwargs):
        ...
        encountered. The regular expression must have two named capture groups
        ``current`` and ``max`` used to determine the current progress.
        ...
        with ui.progress() as progress:
            for line in child.stdout:
                ...
                try:
                    ...
                    progress(int(m.group('current')) / int(m.group('max')))
                except StopIteration:
                    pass
        code = child.wait()
        if code != 0:
...
class Web:
    ...
    def _text(
            self, url: str, content_type_default: str,
            content_type_check: str=None) -> str:
        ...
        """
        with self._open(url) as c:
            content_type = c.headers.get('content-type', content_type_default)
            if content_type.startswith(
                    content_type_check or content_type_default):
                return c.read().decode(self._encoding(content_type))
            else:
                raise NestException(
    ...
    def _encoding(self, content_type: str, default: str='utf-8') -> str:
        """Attempts to parse the encoding from a Content-Type header.

        :param content_type: The value of the header.
        :param default: A default value to return if the encoding cannot be
        detected.
