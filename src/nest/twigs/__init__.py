...
__path__ = [str(TWIG_PATH)]
__all__ = tuple(p.name for p in TWIG_PATH.iterdir() if p.is_dir())
...
class Twig:
    def __init__(
        ...
        self._implementation = [
            ...
            if os.path.isfile(f.filename) and Path(f.filename)
                .is_relative_to(TWIG_PATH)]
        ...
        self._provides = set()
        self._source = source
        self._copy_files = not any(twig._source == source for twig in TWIGS)
    ...
    @property
    def implementation(self) -> list[Path]:
        """The files implementing this twig."""
    ...
    def stored_version(self) -> str:
        ...
        try:
            ...
            raise NestException(
                'The file {} must contain the twig version', str(path)
            )
    ...
    def dependencies(self) -> Set['Self']:
        ...
        try:
            return {
                next(f for f in TWIGS if f.name == dependency)
                for dependency in self._dependencies
            }
        except StopIteration:
            raise NestException(
                'Twig {} has unmet dependencies: {} ' '(available twigs: {})',
                str(self),
                ', '.join(
                    n
                    for n in self._dependencies
                    if not any(f.name == n for f in TWIGS)
                ),
    ...
    def system_source(self) -> Path:
        """The source directory for system files."""
        return self._source / SYSTEM_FILES_PATH

    @property
    def user_source(self) -> Path:
        """The source directory for user files."""
    ...
    def enabled(self) -> bool:
        ...
        def alternative_available(name):
            return any(
                name in twig._provides and twig.enabled for twig in TWIGS
            )

        return (
            self.name not in self.configuration.disable
    ...
    def run(
        ...
        **kwargs,
        ...
        ins = subprocess.DEVNULL if not interactive else stdin
        ...
        outs = (
            subprocess.PIPE
            if capture
            else subprocess.DEVNULL if silent else None
        )

        # Perform string interpolation on kwargs for all command arguments
        args = [Twig.interpolate(arg, kwargs.get) for arg in args]

        try:
            env = os.environ
            if env_path is not None:
                env['PATH'] = (
                    os.pathsep.join(env_path) + os.pathsep + env['PATH']
                )
    ...
    def list_files(self, directory: Path) -> List[Path]:
        ...
        for root, dirs, filenames in os.walk(directory):
            ...
            for path in ((root / d).absolute() for d in dirs):
                (a if path in self.configuration.submodules else b).append(path)
            result.extend((root / dir).relative_to(directory) for dir in a)
            result.extend(
...
class Web:
    ...
    def open(self, url: str, encoding_default: Optional[str] = 'utf-8') -> IO:
        ...
        def encoding(content_type) -> str:
            try:
                return next(
                    re.finditer(r'charset=([^ ;]+)', content_type or '')
                ).group(1)
            except StopIteration:
    ...
    **kwargs,
    ...
    def update_lister(me: Twig) -> List[str]:
        if (
            ...
            and latest_version(me) is not None
...
def archive(
    ...
    **kwargs,
    ...
    if 'zip' in extension:
        ...
        raise NestException(
            'unknown archive type for {}: {}', main.name, extension
        )
    ...
    def install(me: Twig):
        ...
        if exclusive:
            for path in (
                path for path in me.list_files(target_dir) if path not in files
            ):
