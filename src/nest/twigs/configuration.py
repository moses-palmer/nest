...
class Container:
    def __init__(self, path: Optional[str], values: dict = {}):
        ...
        self._path = path
        self._values = (
            {normalize(k): v for (k, v) in values.items()} if values else None
        )
...
class Configuration:
    ...
    def __init__(self, gitmodules: Path, *filenames: Path, **env):
        ...
        data = {
            self.ENV_SECTION: {k: v for k, v in env.items() if k[0] != '_'},
        ...
        for filename in filenames:
            ...
            for section, values in values.items():
                if section in (self.ENV_SECTION, self.SUBMODULES_SECTION):
                    raise ValueError(
                        'the section {} is reserved'.format(section)
                    )
                else:
    ...
    def _extract_values(
        self, filename: str, values: Dict[str, Any]
    ) -> Dict[str, str]:
    ...
    def _extract_sections(
        ...
        for key, entries in configuration.items():
            ...
            yield (
                ...
                {normalize(k): v for (k, v) in entries.items()},
            )
