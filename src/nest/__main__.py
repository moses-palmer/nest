...
def dependencies(
    ...
    if invert:
        ...
        leaves = lambda twig: (t for t in TWIGS if any(
            d.name == twig.name
            for d in t.dependencies))
    else:
        ui.log(ui.bold('Dependencies for twigs:'))
        leaves = lambda twig: twig.dependencies

    def display(twig, is_last, level, stops):
        ui.log('{} {}'.format(
            ''.join(
                '   ' if i not in stops else '│  '
                for i in range(level)) + (
                    '├╼' if not is_last else '└╼'),
            (ui.bold if level == 0 else ui.ignoring)(twig.name)))
        twigs = list(leaves(twig))
