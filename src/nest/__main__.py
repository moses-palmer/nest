    ...
    with ui.section(ui.bold('Removing disabled twigs'), delay=True):
        for twig in reversed([t for t in TWIGS if not t.enabled]):
            ...
            with ui.section(ui.bold('Unlinking files'), delay=True):
                user_files = [target / p for p in twig.user_files]
                system_files = [Path.root / p for p in twig.system_files]
                for path in user_files + system_files:
                    name = str(path) \
                        if not path.is_relative_to(directories.HOME) \
                        else path.relative_to(directories.HOME)
                    if path.exists() or path.is_symlink():
                        ui.log(
                            ui.item(ui.removing(name)))
                        twig.unlink(path)
            ...
            with ui.section(header, length=len(header_no_format)):
                ...
                with ui.section('Linking files', delay=True):
                    ...
                    for rel in twig.system_files:
                        ui.link(twig, twig.system_source, Path.root, rel)
