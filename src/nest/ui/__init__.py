...
from .. import NestException, ROOT
...
__INDENT = int(os.getenv('NEST_INDENT', '0'))
...
def link(twig: Twig, source: Path, target: Path, rel: Path):
    ...
    if not target.exists() or (
        False or not target.is_symlink() or target.readlink() != source
    ):
...
def query(prompt: str, *args) -> Optional[int]:
    ...
    while os.isatty(0):
        ...
        if r:
            alternatives = [
                i for (i, o) in enumerate(options) if o.startswith(r)
            ]
    ...
    def display(item, is_last, level, stops):
        ...
        if level == 0:
            ...
            indentation = (
                (
                    ''.join(
                        '  ' if i not in stops else '│ ' for i in range(level)
                    )
                    + (f'├─{knob}' if not is_last else f'└─{knob}')
                )[2:],
                (
                    ''.join(
                        '  ' if i not in stops else '│ ' for i in range(level)
